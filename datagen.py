...
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))  # Get the directory where `datagen.py` is located
config = {"root": os.path.join(SCRIPT_DIR, "data")}  # Ensure data is created inside `tds/data`

# Ensure the `/data/` directory exists inside `tds`
...
#     large_font = ImageFont.truetype("arial.ttf", 80)  # Use Arial with bigger font
#     large_font.size = 60
...
def a8_credit_card_image():
    """Generate a credit card image and extract the number using OCR."""
    data = get_credit_card(config["email"])
    ...
    # Draw text
    draw.text((100, 300), cc_number, fill=(255, 255, 255), font=large_font)

    # Save image before running OCR
    image_path = os.path.join(config["root"], "credit_card.png")
    image.save(image_path)
    print(f"✅ Credit Card Image Saved at: {image_path}")

    # Ensure image file is saved before OCR
    time.sleep(1)  # Small delay to prevent race condition

    # OCR Extraction (use image_path instead of image object)
    extracted_text = pytesseract.image_to_string(image_path, config="--psm 7").strip()
    print(f"🔍 OCR Extracted Text: {extracted_text}")  # Debugging: Check OCR output
    ...
    if extracted_numbers:
        ...
        print(f"🔍 Extracted Credit Card Number: {card_number}")  # Debugging: Check extracted number

        # Ensure extracted number is 16 digits
        if len(card_number) == 16:
            write_file("credit-card.txt", card_number)
            print(f"✅ Credit Card Number Saved in: /data/credit-card.txt")
        else:
            print("❌ Error: Extracted card number is not 16 digits.")
    else:
        print("❌ Error: No valid credit card number detected in the image.")
...
def a10_ticket_sales():
    ...
    if os.path.exists(target):
        os.remove(target)
    if os.path.exists(target):
        os.remove(target)  # Ensure the file is deleted before recreating
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", default=os.path.join(SCRIPT_DIR, "data"))  # ✅ Use correct data path
    args = parser.parse_args()

    config["email"] = args.email
    config["root"] = args.root  # ✅ Don't overwrite with absolute path

    os.makedirs(config["root"], exist_ok=True)  # Ensure the directory exists

    print("DISCLAIMER: THIS SCRIPT WILL CHANGE BEFORE THE EVALUATION. TREAT THIS AS A GUIDE.")
    print("Files created at", config["root"])  # ✅ Should now be inside `tds/data`
